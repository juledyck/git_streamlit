...
secret_credentials = st.secrets["credentials"]
secret_credentials_dict = json.loads(secret_credentials)
...
def generate_recipe(jpeg, name, recipe, num, recipe_key):
    st.image(jpeg)
    st.write(name)
    if st.button(f"View more ➜", key=recipe_key):
        st.session_state['selected_recipe' + num] = recipe

def show_selected_recipe1(button):
    if 'selected_recipe2' in st.session_state:
            del st.session_state ['selected_recipe2']
    if 'selected_recipe1' in st.session_state:
        ...
        st.write("\n")
        if st.button("Schließen", key=button):
            del st.session_state['selected_recipe1']

def show_selected_recipe2(button):
    if 'selected_recipe1' in st.session_state:
            del st.session_state ['selected_recipe1']
    if 'selected_recipe2' in st.session_state:
        st.markdown("<hr>", unsafe_allow_html=True)
...
def my_chat(messagi, stream=True):
...
if st.session_state['authentication_status']:
    colo, colu, coli = st.columns([4,2,1])
    
    with colu:
        st.write("")
        st.write("")
        st.write(f'Wilkommen *{st.session_state["name"]}*')
    with coli:
        st.write("")
        st.write("")
    ...
    if(st.button("Enter")):
        ...
        for part in my_chat(recipe_prompt):
            recipe += part
            if sprache == "Englisch":
        ...
        if sprache == "Deutsch":
            ...
            translate_prompt = f"""
                ...
                """
            translate_recipe = ""
            for d in my_chat(translate_prompt):
                translate_recipe += d
                recipe_container.markdown(
                f"""
                <div style="background-color: #FFDDC1; padding: 10px; border-radius: 5px;">
